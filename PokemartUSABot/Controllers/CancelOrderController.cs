...
namespace PokemartUSABot.Controllers
{
    ...
    public class CancelOrderController : ControllerBase
    {
        ...
        {
            PokemartUSABot.Logger.LogDebug("Received google sheets cancel order webhook: {payload}", payload.ToString());
            ...
            if (member != null)
            {
                ...
                DiscordMessageBuilder resultMessage = new DiscordMessageBuilder().AddEmbed(PokemartUSABot.CreateOrderDm($"Order '{payload.Data["Row Number"]}' Canceled", payload, payload.Reason));
